...
{
로그폼을 닫으면 메모리에서 해제하는 것이 아니고, 단지 Hide 시킨다.
그리고 시스템 Tray에 icon으로 보인다. 이 아이콘을 우클릭하면,
등록된 popup 메뉴를 볼 수 있다.


소스 컬럼을 추가한 PrintfHandler는 이전 버전의 모든 기능을 유지하면서 다음과 같은 기능이 새롭게 추가되었습니다:

소스 컬럼 표시
로그 메시지에 소스 정보를 추출하여 별도 컬럼으로 표시
...
소스별 색상 지정
소스별로 다른 색상 적용 가능
ColorBySource 속성으로 활성화/비활성화 가능


소스 통계 기능
소스별 로그 수와 오류/경고 수 통계 표시
비율 정보도 함께 제공


컬럼 가시성 설정
모든 컬럼(시간, 레벨, 소스, 메시지)의 표시/숨김 설정 가능
Ctrl+V 단축키로 컬럼 설정 패널 접근


소스 컬럼 컨텍스트 메뉴
소스 컬럼 헤더에서 우클릭하여 소스별 필터링 메뉴 표시


파일 저장 및 로드 시 소스 정보 처리
소스 정보를 포함한 로그 저장
저장된 로그 파일에서 소스 정보 파싱

이 코드는 기존 VirtualTreeView 리팩토링 코드와 호환되며, 소스 정보를 통해 로그의 출처를 더 명확하게 파악할 수 있게 해줍니다.
}
